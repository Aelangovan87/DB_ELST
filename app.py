...
class ProjectForm(FlaskForm):
    ...
    customer_name = StringField('Customer Name', validators=[Optional()])  # <-- add this line
...
def index():
    ...
    for project in projects:
        # Get all delivery addresses for this project (by cust_no)
        deladdrs = db.execute('SELECT address_copies FROM deladdress WHERE cust_no = ?', (project['cust_no'],)).fetchall()
        # Sum up all address_copies (make sure to handle NULL/empty/None)
        total_copies = 0
        for addr in deladdrs:
            try:
                ...
                copies = 0
            total_copies += copies
        # Convert Row to dict so we can add new field
...
def elsts():
    ...
    # List all ELSTs (optionally filter by cust_no)
...
def extract_elst_data():
    ...
    try:
        # Read PDF bytes for use by both PyPDF2 and pdfplumber
        file_bytes = file.read()
        
        # --- Extract text (first 2 pages) and ELST No ---
        ...
        # --- Extract tables (drawings) using pdfplumber ---
        drawings = []
        table_data = []
        with pdfplumber.open(io.BytesIO(file_bytes)) as pdf:
            for i, page in enumerate(pdf.pages[:2]):
                ...
                for table in tables:
                    # Simple heuristic: If the table has headers like "Drawing No" or "Title", treat as drawings
                    ...
                    if any("drawing" in h for h in headers) and any("title" in h for h in headers):
                        ...
                        table_data = table  # Save the first found table for tabular display

        result = {
            'elst_no': elst_no,
            'message': "ELST No extracted." if elst_no else "ELST No not found in PDF.",
            'extracted_text': text,
            'table_data': table_data,   # for tabular view
            'drawings': drawings        # for drawings table view
        }
